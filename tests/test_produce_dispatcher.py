...
async def test_link_success(produce_dispatcher: ProduceDispatcher) -> None:
    correlation_id = 1
    futures = [asyncio.Future(), asyncio.Future()]

    await produce_dispatcher.link(correlation_id=correlation_id, futures=futures)
...
async def test_link_duplicate_correlation_id(
    produce_dispatcher: ProduceDispatcher,
) -> None:
    correlation_id = 1
